...
player
	verb
		...
		new_modifier()
			var/stat/s = input("Stat") in stats
			...
			s.addMod(new/mod("Player-made", src, target, kind, value))
	...
	Login()
		...
		for(var/count = 0, count < 50, count ++)
			...
			var/slot = pick("Helmet", "Armor", "Left Hand", "Right Hand", "Left Ring", "Right Ring", "Amulet", "Belt")
	...
	Stat()
			...
			for(var/stat/s in stats)
				...
				for(var/mod/m in s.mods)
					if(m.targetVar == STAT_VALUE)
						if(m.kind == MOD_FLAT)
							stat(">[m.source.name] -> [m.value]")
						else if(m.kind == MOD_PROP)
							stat(">[m.source.name] -> [m.value*100]%")
					else if(m.targetVar == STAT_MAX_VALUE && istype(s, /stat/bar))
						if(m.kind == MOD_FLAT)
							stat(">[m.source.name] -> [m.value] max")
						else if(m.kind == MOD_PROP)
							stat(">[m.source.name] -> [m.value*100]% max")
					else if(m.targetVar == STAT_MIN_VALUE && istype(s, /stat/bar))
						if(m.kind == MOD_FLAT)
							stat(">[m.source.name] -> [m.value] min")
						else if(m.kind == MOD_PROP)
							stat(">[m.source.name] -> [m.value*100]% min")
		...
		if(statpanel("Blacksmith"))
			for(var/equipment/q in contents)
				if(q.equipped == null)
					stat(q)
					stat("[q.slot] - [list2params(q.costs)]")