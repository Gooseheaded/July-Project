...
GUIObject
	...
	proc
		move(px, py)
			...
			if(offsetX >= wx && screenX < screen_x)
				screenX ++
				offsetX -= icon_x

			if(offsetX < -wx && screenX > 1)
				screenX --
				offsetX += icon_x

			if(offsetY >= wy && screenY < screen_y)
				screenY ++
				offsetY -= icon_y

			if(offsetY < -wy && screenY > 1)
				screenY --
				offsetY += icon_y
		...
		update(callChild = 1)
			...
			if(callChild)
				for(var/GUIObject/gui in children) gui.update()
